...
в path), `GET /api/traffic/top-domains`. Строки устройств обогащаются на чтении
двумя источниками: вендором через чистый локальный `clients/discovery.LookupVendor`
(без БД/сети) и — для mac-устройств — «дружелюбным» hostname из таблицы
`host_names`, которую наполняет фоновый mDNS-сборщик (см. `clients/hostnames/`,
§ ниже). Резолв hostname — best-effort: один JOIN-в-памяти на запрос
(`HostnameFunc` → `Repo.AllAsMap`), а сбой чтения отдаёт пустую карту и логируется,
не роняя дашборд. Фронт выбирает заголовок устройства по приоритету
hostname → vendor → IP, при этом MAC/IP всегда остаётся в подзаголовке.
...
**Hostname-сборщик (`clients/hostnames/`).** Фоновая горутина (только LAN-режим,
стартует рядом с arpwatcher), которая раз в `DefaultInterval` (10 мин) делает
mDNS-браузинг сети (`discovery.BrowseMDNS` — лёгкий вариант discovery **без**
тяжёлого active-ARP-скана) и для каждого анонсированного `IP→имя` резолвит IP в
MAC через тот же `arpwatcher`, после чего пишет `MAC→hostname` в таблицу
`host_names` (`clients/hostnames/db`). **Ключ — только MAC:** MAC устройства
стабилен на время его ассоциации с сетью (даже privacy-рандомизированный),
тогда как IP уходит с DHCP-арендой — ключ по IP «приклеил» бы имя к адресу,
который DHCP потом отдаст другому устройству. Хост с неизвестным на момент свипа
MAC просто пропускается (доучивается на следующем свипе), а не пишется под IP.
Покрытие частичное по природе: устройства, ничего не вещающие по mDNS (часть
Android-телефонов, многое IoT), имени тут не получают и в UI откатываются на
vendor/IP. Свип сам ставит себе таймаут (~5s в `BrowseMDNS`), частичная ошибка
mDNS логируется, но не отменяет свип; departed-устройства подчищаются
`PruneOlderThan(DefaultTTL=30д)` после каждого прогона. Это потребитель/комплемент
к on-demand сканеру `POST /api/clients/discover` (та же mDNS-машинерия, но по
кнопке и без сохранения).
