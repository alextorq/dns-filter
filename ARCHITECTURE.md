...
2. Дедупликация.
3. Пакетный **upsert** в `block_lists` (`CreateDNSRecordsByDomains`).

`Sync()` только вставляет/обновляет — он **не удаляет** записи, исчезнувшие
из источника. Чтобы выкинуть из `block_lists` отравленные записи, оставшиеся
от прошлой версии парсера, источник нужно пересобрать вручную: удалить его
строки и повторить `Sync`.
