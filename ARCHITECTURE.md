...
**`traffic`** ниже). Статистика блокировок (`/api/events/block/*`) теперь
считается как `SUM(count) WHERE blocked=true` из `domain_traffic` через
`BlockStatsAdapter` — JSON-форма ответа сохранена, фронтенд не затронут.
...
`DomainsForDevice`, `TopDomains`); статистику блокировок
(`CountByDomain`/`TotalCount` через `BlockStatsAdapter`); пул кандидатов suggest
