...
- **Singletons via `sync.Once`** still provide the bloom filter, logger, DNS cache, and `config.GetConfig()`. DI migration is incremental: DNS is constructed with `dns.NewServer(dns.ServerDeps{...})`; `clients.Module` receives `clients/db.Repo` plus a per-process exclusion store, while its ARP cache/watcher are also constructed in `main`. Neither feature reaches into config/DB/store singletons internally.
