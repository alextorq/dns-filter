...
4. Background goroutines: `traffic` retention prune (daily — `traffic_prune.Run`, the sole retention task now that the per-table `clear-events` are gone), `suggestModule.Start` (12h cron), `authBusiness.ClearExpiredSessions`. **LAN mode only:** `arpwatcher.Run` (IP↔MAC cache) and `hostnames.Collector.Run` (10-min mDNS sweep that learns MAC→hostname into the `host_names` table to enrich device names in the traffic dashboard — see `clients/hostnames/`).
