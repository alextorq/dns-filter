...
- **Singletons via `sync.Once`** still provide the bloom filter, logger, DNS cache, client exclusion store, and `config.GetConfig()`. DI migration is incremental: the DNS package is already isolated and must be constructed with `dns.NewServer(dns.ServerDeps{...})`, so it never calls config/store getters internally; `clients` still resolves its store singleton inside CRUD use-cases and the ARP watcher until that feature gets its own `Module`.
