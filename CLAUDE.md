...
4. Background goroutines: `traffic` retention prune (daily — `traffic_prune.Run`, the sole retention task now that the per-table `clear-events` are gone), `suggestModule.Start` (12h cron), `authBusiness.ClearExpiredSessions`.
