...
1. `conn := db.GetConnection()` followed by `migrate.Migrate(conn)` — the composition root resolves the shared connection once and passes it explicitly into schema migrations
