...
- Web handlers declare consumer-owned ports for persistence/services instead of storing concrete DB adapters. `source/web.SourceRepo` is the basic reference pattern; `blocked-domain/web.Handlers` additionally splits list/create/update dependencies so each endpoint sees only the port it needs. Production `*Repo` values satisfy these ports structurally, and handler tests use fakes without SQLite where persistence behavior is not under test.
