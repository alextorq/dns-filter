...
- Web handlers declare consumer-owned ports for persistence/services instead of storing concrete DB adapters. `source/web.SourceRepo` is the reference pattern: only methods used by the handlers belong in the interface, production `*Repo` satisfies it structurally, and handler tests use fakes without SQLite.
