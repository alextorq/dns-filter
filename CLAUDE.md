...
1. `conf := config.GetConfig()`, an explicit `ChanLogger`, one `prometheus.Registry` plus component `NewMetrics` bundles, then `conn, err := db.Open(db.OpenDeps{Path, Log, Metrics, DBName})` followed by `migrate.Migrate(conn)` — the composition root creates the process logger, metrics and shared connection once, assigns the DB pool a unique metrics label, handles startup errors, and passes the connection explicitly into schema migrations
...
- **Process-owned state.** `main` explicitly loads configuration, constructs the logger, opens the shared DB, and creates the bloom filter, filter verdict LRU and DNS response cache before passing them to consumers. `config.GetConfig()` remains the process-boundary env loader; DB and logger have no production singleton accessor. DI migration is incremental: DNS is constructed with `dns.NewServer(dns.ServerDeps{...})`; the DNS response cache is created once in `main` and passed to DNS, settings and `dns-cache/web.Handlers`; `clients.Module` similarly receives `clients/db.Repo` plus per-process store/cache instances. These features do not resolve application dependencies internally.
