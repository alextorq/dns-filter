...
4. Background goroutines: explicit `db.DBSizeMonitor.Run(ctx)` (immediate + 10-minute SQLite file-size samples; constructed in `main`, no `init()` goroutine), `traffic` retention prune (daily — `traffic_prune.Run`, the sole retention task now that the per-table `clear-events` are gone), `suggestModule.Start` (12h cron), `authModule.ClearExpiredSessions`, **always launched** `suggest_inspect.Worker.Start` + `StartPrune` (reputation-enrichment of weak suggest candidates — `Worker.RunOnce` is a no-op until the DB-backed `suggest_inspect_enabled` toggle is on, and the VT/SB keys are themselves DB-backed `secret` settings; see `suggest-to-block/inspect/` and ARCHITECTURE.md §11). The shared `periodic.Run` takes its context and logger explicitly; it performs one immediate cleanup, logs failures without stopping, and on cancellation stops scheduling new work after any in-flight callback finishes. The inspect goroutines **and the daily `traffic_prune.Run` retention prune** are spawned **after** `settingsModule.HydrateAll()` so their injected in-memory states (the inspect toggle/keys and `traffic_retention_days`) already reflect the DB override on the first tick — the prune's first run is immediate, and pruning with the pre-hydrate seed could hard-delete rows using a larger override meant to keep (`RetentionState` starts at a `0` "unset" sentinel that `pruneTaskAt` skips, as belt-and-suspenders). **LAN mode only:** `arpwatcher.Run` (IP↔MAC cache) and `hostnames.Collector.Run` (10-min mDNS sweep that learns MAC→hostname into the `host_names` table to enrich device names in the traffic dashboard — see `clients/hostnames/`).
5. `backgroundSync` goroutine — `sourceModule.Sync(ctx)` pulls block lists from external sources (Steven Black, EasyList, …) into the DB, then re-runs `filterModule.UpdateFromDb()` to rebuild the bloom and clear the verdict cache. It runs **in the background** so the DNS server starts without waiting on network I/O. It never panics — a panic here would kill an already-serving DNS server; instead a failed sync (typically no network on first boot) is retried with exponential backoff (`syncRetryBaseDelay` → `syncRetryMaxDelay`) until it succeeds. The context reaches both HTTP loaders and GORM insert/delete batches, loader-facing parsers surface `scanner.Err` so a canceled streaming body is never accepted as a complete list, and the retry wait uses a timer/select rather than `time.Sleep`; cancellation skips further retries, DB writes, filter refresh and success logging. The composition root currently passes `context.Background()` and will replace it with the signal-derived app context in the graceful-shutdown step.
