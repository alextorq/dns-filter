...
5. The `backgroundSync` job — `sourceModule.Sync(ctx)` pulls block lists from external sources (Steven Black, EasyList, …) into the DB, then re-runs `filterModule.UpdateFromDb()` to rebuild the bloom and clear the verdict cache. `main` owns an explicit timeout-configured HTTP client and passes it to `source_sync.NewDefaultLoaders`; the feature factory owns the stable source→adapter/endpoint mapping and returns the validated registry injected into `source.Module`. Source sync has no package-level HTTP client. It runs **in the background** so the DNS server starts without waiting on network I/O. It never panics — a panic here would kill an already-serving DNS server; instead a failed sync (typically no network on first boot) is retried with exponential backoff (`syncRetryBaseDelay` → `syncRetryMaxDelay`) until it succeeds. The context reaches both HTTP loaders and GORM insert/delete batches, loader-facing parsers surface `scanner.Err` so a canceled streaming body is never accepted as a complete list, and the retry wait uses a timer/select rather than `time.Sleep`; cancellation skips further retries, DB writes, filter refresh and success logging.
