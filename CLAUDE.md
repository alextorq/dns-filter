...
1. `conf := config.GetConfig()`, an explicit `ChanLogger`, then `conn, err := db.Open(db.OpenDeps{Path, Log, Registerer, DBName})` followed by `migrate.Migrate(conn)` — the composition root creates the process logger and shared connection once, assigns the DB pool a unique metrics label, handles startup errors, and passes the connection explicitly into schema migrations
...
7. Prometheus startup is also explicit: `main` calls `metric.RegisterRuntimeCollectors` after the logger exists, constructs `metric.NewServer(":"+MetricPort, metric.Registry)` only when enabled, and starts it after component collectors are registered. `metric` does not read config/logger or open a listener from `init()`; the shared registry remains temporarily global for feature collectors.
