...
- **DI-фичи** получили метод `(h *Handlers) RegisterRoutes(rg *gin.RouterGroup)`:
  `auth/web`, `blocked-domain/web`, `clients/web`, `db/web`, `filter/web`, `logger/web`,
...
- **Package-level фичи** получили функцию пакета `Register(rg *gin.RouterGroup)`:
  `dns-cache/web`, `domain-inspect/web`.
...
| 3 | DI вместо singleton'ов | **готово для core, db/web, auth и clients**. Остатки: `domain-inspect`, `dns-cache` — отдельные PR |
