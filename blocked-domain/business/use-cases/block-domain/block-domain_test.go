...
func (f *fakeRepo) BatchCreateBlockDomainEvents(domains []string) error {
	f.mu.Lock()
	defer f.mu.Unlock()
...
func TestEventStore_FlushesOnCapacity(t *testing.T) {
	repo := &fakeRepo{}
	store := NewBlockDomainEventStore(repo, &recordingLog{}, 2)

	store.SendBlockDomainEvent("a")
	store.SendBlockDomainEvent("b") // hits capacity
...
func TestEventStore_LogsRepoError(t *testing.T) {
	...
	log := &recordingLog{}
	store := NewBlockDomainEventStore(repo, log, 1)

	store.SendBlockDomainEvent("oops")
...
// channel saturates. enter signals the worker entered the flush, exit holds it
// there until the test releases it.
type blockingRepo struct {
	enter chan struct{}
	exit  chan struct{}
}

func (b *blockingRepo) BatchCreateBlockDomainEvents(domains []string) error {
	b.enter <- struct{}{}
	<-b.exit
	return nil
}

// SendBlockDomainEvent must DROP (not block) when the inbox is saturated, and
...
func TestEventStore_DropsWhenChannelFull(t *testing.T) {
	...
	store.SendBlockDomainEvent("first")
	...
	store.SendBlockDomainEvent("second")
	// Third send must hit the default branch → warn + drop.
	store.SendBlockDomainEvent("dropped")
